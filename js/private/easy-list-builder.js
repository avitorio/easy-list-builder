...
jQuery(document).ready(function($){
	...
	*/
	...
	// check if form 2 validates on all changes
	...
	$(document).on( 'change', '#import_subscribers #import_form_2 .elb-input', function(){
		...
		setTimeout( function(){ 
			...
			if ( elb_form_2_is_valid()) {
				...
				// show .show-only-on-valid elemnts
				$('.show-only-on-valid', $import_form_2).show();

			} else {

				// hide .show-only-on-valid elemnts
	...
	// return custom select html with the subscribers data header columns as options
	function elb_get_selector( input_id, subscribers ) {
		...
		// loop over the subscribers[0] data to get headers
		$.each( subscribers[0], function( key, value ) {
	...
	// check if import 2 form is valid
